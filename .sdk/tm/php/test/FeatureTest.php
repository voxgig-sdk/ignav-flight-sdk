...
class FtCtrl extends IgnavFlightControl
{
...
class FtClient
{
    ...
    private IgnavFlightUtility $utility;

    public function __construct(IgnavFlightUtility $utility, array $options)
    {
    ...
    public function get_utility(): IgnavFlightUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IgnavFlightContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IgnavFlightContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IgnavFlightContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IgnavFlightConfig::make_feature($name);
    ...
    private static function buildUrl(IgnavFlightSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IgnavFlightError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IgnavFlightResponse($fetched);
        }
    ...
    private function fail(IgnavFlightContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IgnavFlightContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
