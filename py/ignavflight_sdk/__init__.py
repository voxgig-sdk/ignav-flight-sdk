...
from ignavflight_sdk.utility import register

# Load features
from ignavflight_sdk.feature.base_feature import IgnavFlightBaseFeature
from ignavflight_sdk.features import _has_feature, _make_feature
...
class IgnavFlightSDK:
    ...
    def get_utility(self):
        return IgnavFlightUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("IgnavFlightSDK: graphql: " + str(msg))
            res["graphql"] = errors
    ...
    def Airport(self, data=None) -> "AirportEntity":
        ...
        from ignavflight_sdk.entity.airport_entity import AirportEntity
        return AirportEntity(self, data)


    def BookingLink(self, data=None) -> "BookingLinkEntity":
        """Entity factory: client.BookingLink().list() / client.BookingLink().load({"id": ...})."""
        from ignavflight_sdk.entity.booking_link_entity import BookingLinkEntity
        return BookingLinkEntity(self, data)


    def FareSearchModel(self, data=None) -> "FareSearchModelEntity":
        """Entity factory: client.FareSearchModel().list() / client.FareSearchModel().load({"id": ...})."""
        from ignavflight_sdk.entity.fare_search_model_entity import FareSearchModelEntity
        return FareSearchModelEntity(self, data)


    def FareSearchResponseModel(self, data=None) -> "FareSearchResponseModelEntity":
        """Entity factory: client.FareSearchResponseModel().list() / client.FareSearchResponseModel().load({"id": ...})."""
        from ignavflight_sdk.entity.fare_search_response_model_entity import FareSearchResponseModelEntity
        return FareSearchResponseModelEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "IgnavFlightSDK":
        if sdkopts is None:
            sdkopts = {}
