...
class TestBookingLinkEntity:

    def test_should_create_instance(self):
        testsdk = IgnavFlightSDK.test(None, None)
        ent = testsdk.BookingLink(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _booking_link_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "booking_link." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IGNAV_FLIGHT_TEST_BOOKING_LINK_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        booking_link_ref01_ent = client.BookingLink(None)
        booking_link_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.booking_link"), "booking_link_ref01"))

        booking_link_ref01_data = helpers.to_map(runner.entity_data(booking_link_ref01_ent.create(booking_link_ref01_data, None)))
        assert booking_link_ref01_data is not None



def _booking_link_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/booking_link/BookingLinkTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "IGNAV_FLIGHT_TEST_BOOKING_LINK_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("IGNAV_FLIGHT_TEST_BOOKING_LINK_ENTID"))
