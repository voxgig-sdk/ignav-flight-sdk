...
class TestAirportDirect:

    def test_should_direct_list_airport(self):
        setup = _airport_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-airport", "live" if setup["live"] else "unit")
        if _skip:
...
def _airport_direct_setup(mockres):
