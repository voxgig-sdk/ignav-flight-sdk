...
class FareSearchResponseModelEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "fare_search_response_model";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new FareSearchResponseModelEntity($this->_client, $opts);
    }

    /**
     * @param FareSearchResponseModel|array $args FareSearchResponseModel data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IgnavFlightHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return FareSearchResponseModel|array The current FareSearchResponseModel data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of FareSearchResponseModel fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IgnavFlightHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of FareSearchResponseModel fields).
     */
    ...
    /**
     * Create a new FareSearchResponseModel.
     *
     * @param FareSearchResponseModelCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed FareSearchResponseModelCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return FareSearchResponseModel|array The created FareSearchResponseModel as an assoc-array at the
     *   SDK boundary; throws IgnavFlightError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = IgnavFlightHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
