...
The Lua SDK for the IgnavFlight API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Airport()` — each with the same small set of operations (`list`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/ignav-flight-sdk/releases)),
...
Create an instance: `local booking_link = client:BookingLink(nil)`
...
```lua
local booking_link, err = client:BookingLink():create({
})
```
...
Create an instance: `local fare_search_model = client:FareSearchModel(nil)`
...
Create an instance: `local fare_search_response_model = client:FareSearchResponseModel(nil)`
...
```
lua/
├── ignav-flight_sdk.lua    -- Main SDK module
...
The main module (`ignav-flight_sdk`) exports the SDK constructor
