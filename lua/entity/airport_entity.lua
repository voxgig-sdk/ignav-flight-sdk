...
local AirportEntity = {}
AirportEntity.__index = AirportEntity


function AirportEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AirportEntity)
  self._name = "airport"
  self._client = client
...
function AirportEntity:get_name()
  return self._name
end


function AirportEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AirportEntity.new(self._client, opts)
end
...
function AirportEntity:mark_deleted()
  self._deleted = true
end


function AirportEntity:deleted()
  return true == self._deleted
end


function AirportEntity:data_set(args)
  if args ~= nil then
...
function AirportEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AirportEntity:match_set(args)
  if args ~= nil then
...
function AirportEntity:match_get()
...
function AirportEntity:stream(action, args, callopts)
...
function AirportEntity:_run_op(ctx, post_done)
  local utility = self._utility
