...
local FareSearchModelEntity = {}
FareSearchModelEntity.__index = FareSearchModelEntity


function FareSearchModelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FareSearchModelEntity)
  self._name = "fare_search_model"
  self._client = client
...
function FareSearchModelEntity:get_name()
  return self._name
end


function FareSearchModelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FareSearchModelEntity.new(self._client, opts)
end
...
function FareSearchModelEntity:mark_deleted()
  self._deleted = true
end


function FareSearchModelEntity:deleted()
  return true == self._deleted
end


function FareSearchModelEntity:data_set(args)
  if args ~= nil then
...
function FareSearchModelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FareSearchModelEntity:match_set(args)
  if args ~= nil then
...
function FareSearchModelEntity:match_get()
...
function FareSearchModelEntity:stream(action, args, callopts)
...
---@param reqdata FareSearchModelCreateData
---@param ctrl? table
---@return FareSearchModel
---@return string? err
function FareSearchModelEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function FareSearchModelEntity:_run_op(ctx, post_done)
  local utility = self._utility
