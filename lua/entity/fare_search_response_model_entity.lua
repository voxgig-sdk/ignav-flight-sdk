-- IgnavFlight SDK FareSearchResponseModel entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local FareSearchResponseModelEntity = {}
FareSearchResponseModelEntity.__index = FareSearchResponseModelEntity


function FareSearchResponseModelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FareSearchResponseModelEntity)
  self._name = "fare_search_response_model"
  self._client = client
...
function FareSearchResponseModelEntity:get_name()
  return self._name
end


function FareSearchResponseModelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FareSearchResponseModelEntity.new(self._client, opts)
end
...
function FareSearchResponseModelEntity:mark_deleted()
  self._deleted = true
end


function FareSearchResponseModelEntity:deleted()
  return true == self._deleted
end


function FareSearchResponseModelEntity:data_set(args)
  if args ~= nil then
...
function FareSearchResponseModelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FareSearchResponseModelEntity:match_set(args)
  if args ~= nil then
...
function FareSearchResponseModelEntity:match_get()
...
function FareSearchResponseModelEntity:stream(action, args, callopts)
...
---@param reqdata FareSearchResponseModelCreateData
---@param ctrl? table
---@return FareSearchResponseModel
---@return string? err
function FareSearchResponseModelEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function FareSearchResponseModelEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return FareSearchResponseModelEntity
