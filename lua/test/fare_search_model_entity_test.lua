...
describe("FareSearchModelEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:FareSearchModel(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = fare_search_model_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "fare_search_model." .. _op, _live and "live" or "unit")
    ...
    -- CREATE
    local fare_search_model_ref01_ent = client:FareSearchModel(nil)
    local fare_search_model_ref01_data = helpers.to_map(vs.getprop(
...
function fare_search_model_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/fare_search_model/FareSearchModelTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read fare_search_model test data: " .. entity_data_file)
  end
