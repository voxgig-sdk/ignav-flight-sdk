...
const TEST_SEED = { "entity": { "airport": { "test01": { "id": "test01" } }, "booking_link": { "test01": { "id": "test01" } }, "fare_search_model": { "test01": { "id": "test01" } }, "fare_search_response_model": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+IgnavFlightSDK\b/.test(code) ||
        /\bIgnavFlightSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+IgnavFlightSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/IgnavFlightSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
