...
module IgnavFlightUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IgnavFlightResponse.new({})
    ...
    if fetch_err
      ...
      response = IgnavFlightResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IgnavFlightResponse.new(fetched)
    else
