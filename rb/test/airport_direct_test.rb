...
class AirportDirectTest < Minitest::Test
  def test_direct_list_airport
    setup = airport_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-airport", setup[:live] ? "live" : "unit")
    if _should_skip
...
def airport_direct_setup(mockres)
