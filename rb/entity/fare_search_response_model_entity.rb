...
class FareSearchResponseModelEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "fare_search_response_model"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    FareSearchResponseModelEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IgnavFlightHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [FareSearchResponseModel, Hash] the current FareSearchResponseModel data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IgnavFlightHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of FareSearchResponseModel fields)
  def match_get
  ...
  #
  # @param reqdata [FareSearchResponseModelCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [FareSearchResponseModel, Hash] the created FareSearchResponseModel; raises IgnavFlightError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = IgnavFlightHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
