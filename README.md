...
Learn more about Ignav Public API at [ignav.com](https://ignav.com).

This is an unofficial SDK for the Ignav Public public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Airport, BookingLink, FareSearchModel and FareSearchResponseModel — that you
...
const items = await client.Airport().list({ q: "example" })
```
...
| --- | --- | --- |
| **Airport** | The Airport entity (list). | `/api/airports` |
| **BookingLink** | The BookingLink entity (create). | `/api/fares/booking-links` |
| **FareSearchModel** | The FareSearchModel entity (create). | `/api/fares/search` |
| **FareSearchResponseModel** | The FareSearchResponseModel entity (create). | `/api/fares/one-way` |
...
$airports = $client->Airport()->list();
print_r(array_map(fn($item) => $item->data_get(), $airports));
```
...
Generated from the Ignav Public API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
