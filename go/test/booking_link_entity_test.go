...
func TestBookingLinkEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IGNAV_FLIGHT_TEST_BOOKING_LINK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		bookingLinkRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "booking_link"}), "booking_link_ref01"))

		bookingLinkRef01DataResult, err := bookingLinkRef01Ent.Create(bookingLinkRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		bookingLinkRef01Data = core.ToMapAny(entityData(bookingLinkRef01DataResult))
...
func booking_linkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IGNAV_FLIGHT_TEST_BOOKING_LINK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IGNAV_FLIGHT_TEST_BOOKING_LINK_ENTID"])
