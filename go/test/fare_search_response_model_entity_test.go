...
func TestFareSearchResponseModelEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IGNAVFLIGHT_TEST_FARE_SEARCH_RESPONSE_MODEL_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		fareSearchResponseModelRef01Data = core.ToMapAny(fareSearchResponseModelRef01DataResult)
...
func fare_search_response_modelBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IGNAVFLIGHT_TEST_FARE_SEARCH_RESPONSE_MODEL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IGNAVFLIGHT_TEST_FARE_SEARCH_RESPONSE_MODEL_ENTID"])
