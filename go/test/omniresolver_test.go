...
import (
	...
	omni "github.com/voxgig-sdk/ignav-flight-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IgnavFlightError); is {
				out := map[string]any{
					"name":    "IgnavFlightError",
...
func omniCtx(arg any, client *sdk.IgnavFlightSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IgnavFlightSDK); is {
			client = live
