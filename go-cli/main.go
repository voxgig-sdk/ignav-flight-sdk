// Command IgnavFlight-cli is a CLI + REPL for the IgnavFlight SDK,
...
const prompt = "ignav-flight"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "airport booking_link fare_search_model fare_search_response_model"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IGNAV_FLIGHT_APIKEY (api key), IGNAV_FLIGHT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IGNAV_FLIGHT_APIKEY carries the API key and
	// IGNAV_FLIGHT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IGNAV_FLIGHT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IGNAV_FLIGHT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIgnavFlightSDK(opts)
