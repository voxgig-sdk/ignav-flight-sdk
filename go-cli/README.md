...
boru-driven command-line client **and** interactive REPL for the IgnavFlight
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ignav-flight-cli)
make build

# 2. See usage (words, entities, env vars)
./ignav-flight-cli --help

# 3. Provide credentials once, via the environment
export IGNAV_FLIGHT_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ignav-flight-cli list airport
./ignav-flight-cli list booking_link

# 5. Override the API base URL for a single call
IGNAV_FLIGHT_BASE=https://api.example.com ./ignav-flight-cli list airport

# 6. No arguments -> interactive REPL
./ignav-flight-cli
ignav-flight> list airport
ignav-flight> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ignav-flight-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ignav-flight-cli list airport
   ```
...
Run with no arguments to open a REPL (prompt `ignav-flight>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `airport`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ignav-flight-cli`. |
...
(`./ignav-flight-cli <expr>`) and interactively (the REPL), and expressions compose the
